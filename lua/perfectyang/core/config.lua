...
vim.keymap.set("n", "<leader>gl", function()
  extcmd("git log --oneline")
end)
vim.keymap.set("n", "<leader>gd", function()
  extcmd("git diff")
end)
vim.keymap.set("n", "<leader>gb", function()
  extcmd("git blame " .. vim.fn.expand("%"), false, false, true)
end)
vim.keymap.set("n", "<leader>gs", function()
  extcmd("git show " .. vim.fn.expand("<cword>"))
end)
vim.keymap.set("n", "<leader>gc", function()
  extcmd("git diff --name-only --diff-filter=U", true)
end)
vim.keymap.set("n", "<leader>gp", function()
  vim.cmd(
    "edit " .. vim.fn.system("python3 -c 'import site; print(site.getsitepackages()[0])'"):gsub("%s+$", "") .. "/."
  )
end)
...
vim.keymap.set("n", "<leader>ss", function()
  vim.ui.input({ prompt = "> " }, function(pat)
    if pat then
      extcmd("grep -in '" .. pat .. "' " .. vim.fn.shellescape(vim.api.nvim_buf_get_name(0)), false)
    end
  end)
end)
...
vim.keymap.set("n", "<leader>sf", function()
  vim.ui.input({ prompt = "> " }, function(pat)
    if pat then
      local path, excludes, parts = pre_search()
      for _, pattern in ipairs(excludes) do
        table.insert(parts, string.format("-path '*%s*' -prune -o", pattern))
      end
      extcmd(
        string.format("find %s %s -path '*%s*' -print", vim.fn.shellescape(path), table.concat(parts, " "), pat),
        true,
        true
      )
    end
...
--   vim.keymap.set("n", "<leader>j" .. l, "'" .. u, { noremap = true, silent = true })
-- end
