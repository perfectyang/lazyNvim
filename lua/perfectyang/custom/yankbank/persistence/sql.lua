...
local dbdir = YB_OPTS.db_path or debug.getinfo(1).source:sub(2):match("(.*/).*/.*/.*/") or "./"
...
function data:insert_yank(yank_text, reg_type, pin)
  ...
  is_pinned = (pin == 1 or pin == true) and 1 or (pin == 0 or pin == false) and 0 or is_pinned

  -- insert entry using the eval method with parameterized query to avoid error on 'data:insert()'
  db:with_open(function()
    db:eval("INSERT INTO bank (yank_text, reg_type, pinned) VALUES (:yank_text, :reg_type, :pinned)", {
...
function data:trim_size()
  if self:count() > max_entries then
    ...
    if e then
      db:with_open(function()
        db:eval("DELETE FROM bank WHERE yank_text = :yank_text", { yank_text = e.yank_text })
      end)
    end
  end
end
